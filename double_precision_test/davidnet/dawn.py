...
parser.add_argument('--double',default=0,type=int)
...
class TSVLogger():
    ...
    def append(self, output):
        epoch, hours, acc = output['epoch'], output['total time']/3600, 0#output['test acc']*100
...
def main():
    ...
    opt = torch.optim.SGD(model.parameters(), lr=0.0, momentum=args.momentum, weight_decay=5e-4*args.batch_size, nesterov=True)
    ...
    args.warm_up_iter = math.ceil(dataset_len*args.warm_up_epoch/(world_size * args.batch_size))
