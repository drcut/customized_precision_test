...
from distributed_utils import dist_init, sum_gradients, DistModule
#####################
# utils
#####################
...
class Batches():
    def __init__(self, dataset, batch_size, shuffle, num_workers=0, drop_last=False,sampler=None,half=None,double=None,dist_=None):
    ...
    def __iter__(self):  
        if self.half == 1:
            return ({'input': x.cuda().half(), 'target': y.cuda()} for (x,y) in self.dataloader)
        elif self.double == 1:
            return ({'input': x.cuda().double(), 'target': y.cuda()} for (x,y) in self.dataloader)
        else:
            return ({'input': x.cuda(), 'target': y.cuda()} for (x,y) in self.dataloader)
...
def batch_norm(num_channels, bn_bias_init=None, bn_bias_freeze=False, bn_weight_init=None, bn_weight_freeze=False,exp=8, man=23):
...
def update(model, optimizer,dist_,loss_scale):
    ...
    if dist_ == 1:
        ...
        for param in model.parameters():
            param.grad.data.copy_(param.grad/loss_scale)
    else:
...
global_step = 0
def train_epoch(model, batches, optimizer, lrs, stats,dist_,warm_up_iter,loss_scale):
    global global_step
    model.train(True)   
    for lr, batch in zip(lrs, batches):
            
        collect(stats, model(batch),dist_)
        update(model, set_opt_params(optimizer, {'lr': lr},warm_up_iter),dist_,loss_scale)
        global_step += 1
    return stats
...
def train(model, lr_schedule, optimizer, train_set, test_set, batch_size=512, 
    ...
    train_batches = Batches(train_set, batch_size, shuffle=True, 
                            num_workers=num_workers, drop_last=drop_last,half=args.half,double=args.double,dist_=args.dist)
    test_batches = Batches(test_set, batch_size, shuffle=False, 
                            num_workers=num_workers,half=args.half,double=args.double,dist_=args.dist)
    ...
    for epoch in range(lr_schedule.knots[-1]):
        ...
        train_stats, train_time = train_epoch(model, train_batches, optimizer, lrs, {'loss': [], 'correct': []},args.dist,args.warm_up_iter,args.loss_scale), t()
        test_stats, test_time = test_epoch(model, test_batches, {'loss': [], 'correct': []},args.dist), t(test_time_in_total)
        summary = {
           'epoch': epoch+1, 
