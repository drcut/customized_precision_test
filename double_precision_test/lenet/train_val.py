...
def main():
    ...
    model.cuda()
    if args.double == 1:
        param_copy = [param.clone().type(torch.cuda.DoubleTensor).detach() for param in model.parameters()]
    else:
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.double == 0:
            for param_1,param_2 in zip(param_copy,list(model.parameters())):
                param_1.backward(param_2.grad.float()/args.loss_scale)
        else:
            for param_1,param_2 in zip(param_copy,list(model.parameters())):
                param_1.backward(param_2.grad.double()/args.loss_scale)
        #optimizer.step()
