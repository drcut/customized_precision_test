...
def update(model, optimizer,dist_,loss_scale):
    ...
    if dist_ == 1:
        ...
        sum_gradients(model,use_APS=True)
...
global_step = 0
def train_epoch(model, batches, optimizer, lrs, stats,dist_,warm_up_iter,param_exp,param_man,loss_scale):
    global global_step
    model.train(True)   
...
def train(model, lr_schedule, optimizer, train_set, test_set, batch_size=512, 
    ...
    for epoch in range(lr_schedule.knots[-1]):
        ...
        train_stats, train_time = train_epoch(model, train_batches, optimizer, lrs, {'loss': [], 'correct': []},args.dist,args.warm_up_iter,args.param_exp,args.param_man,args.loss_scale), t()
